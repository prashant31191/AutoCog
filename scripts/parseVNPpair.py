...
def nprefine(eachphrase):
	...
	word=eachphrase[0][0]
        if word.find('/')<0:
                word=word.strip()
                if len(word)>0:
                	newnpinsent.append(word)
        elif word.find('/')>=0:
                for eachsingleword in word.split('/'):
                        wordtoadd=eachsingleword.strip()
                        if len(wordtoadd)>0:
                        	newnpinsent.append(wordtoadd)
        return newnpinsent

    elif len(eachphrase)==2:
        word1=eachphrase[0][0]
        word2=eachphrase[1][0]
        if word1.find('/')<0 and word2.find('/')<0:
                newnpinsent.append(word1+' '+word2)
        elif word1.find('/')>=0 or word2.find('/')>=0:
                wordlst1=word1.split('/')
                wordlst2=word2.split('/')
                for eachsinglewd1 in wordlst1:
                	for eachsinglewd2 in wordlst2:
                        	wordtoadd1=eachsinglewd1.strip()
                        	wordtoadd2=eachsinglewd2.strip()
                         	newnpinsent.append(wordtoadd1+' '+wordtoadd2)
        return newnpinsent
                                            

    tempstr=''
    for eachword in eachphrase:
    	wordtoadd=eachword[0]
        if wordtoadd.find('/')>=0:
        	wordtoadd=wordtoadd.replace('/',' ')
        tempstr=tempstr+' '+wordtoadd
        tempstr=tempstr.strip()
        newnpinsent.append(tempstr)
    return newnpinsent


def newparseNPverbpair(xmlrootpath):
  rootDir=xmlrootpath
  list_dirs=os.walk(rootDir)
  dircounter=0
  maindict={}
	...
	for d in dirs:  
		...
		for Nroot, Ndirs, Nfiles in new_list_dirs:
			for Nfile in Nfiles:
				...
				if len(newparse_xml(filefullpath))==0:
					continue
				
				#print filefullpath
				npvblst=newparse_xml(filefullpath)
				#print npvblst
				for eachelem in npvblst:
					npwithtyp=eachelem[0]
					verblst=eachelem[1]
					np=nprefine(npwithtyp)
					for eachNP in np:
						for eachverb in verblst:
						 if (eachNP, eachverb) not in npverblst:
							npverblst.append((eachNP, eachverb))
							#print npverblst
						

		maindict[app]=npverblst
                print app
		print npverblst
  f=open(proj_path+'/data/new49183_apps_NPverb.json','w')
...
def parseNPverbpair(xmlrootpath):
  rootDir=xmlrootpath
  list_dirs=os.walk(rootDir)
  dircounter=0
  maindict={}
	...
	for d in dirs:  
		...
		for Nroot, Ndirs, Nfiles in new_list_dirs:
			for Nfile in Nfiles:
				...
				npinsent=parse_xml(filefullpath)
                                newnpinsent=[]
                                
                                for eachphrase in npinsent:
                                    if len(eachphrase)==1:
                                        word=eachphrase[0][0]
                                        if word.find('/')<0:
                                            word=word.strip()
				...
				for eachNP in newnpinsent:
                                        if eachNP.find('/')>0:
                                            print eachNP
					for eachverb in verbinsent:
						if (eachNP,eachverb) not in npverblst and  (eachNP,eachverb) not in npverbinsent:
						    npverbinsent.append((eachNP,eachverb))
                                npverblst.extend(npverbinsent)
                                #if len(npverbinsent)!=len(newnpinsent)*len(verbinsent):
                                    #print newnpinsent
                                    #print verbinsent
                                    #print npverbinsent
                                    #print '\n'
		maindict[app]=npverblst
  f=open(proj_path+'/evaluation/22458_apps_NPverb.json','w')
  f.write(json.dumps(maindict))
  f.close()
...
def getverbNPpair(xmlrootpath,NPtocheck,permdict,replaceworddict,trainapplst,fold):
  ...
  for eachPerm in NPtocheck:
	...
	respath=proj_path+'/evaluation/potentVerbNP/'
	resdirpath=respath+eachPerm
	command='mkdir '+resdirpath
	os.system(command)
	targetfilepath=resdirpath+'/VerbNPfold%s.txt'%fold
...
def verblstnormal(verblst):
    ...
    for word in verblst:
	if len(word)>0 and word.lower() not in stopwords and word.lower() not in filteredlist:
...
######
	...
	if len(NP)>0:
		...
		verblst=[]
		tempNP=eachNP
		while tempNP.parentNode.nodeName!='ROOT':
			father=tempNP.parentNode
			
			slib= father.childNodes
			for eachslib in slib:
				
				if eachslib.nodeName.startswith('V'):
					#print eachslib.nodeName
					for eachverbchild in eachslib.childNodes:
						if len(eachverbchild.childNodes)==0:
							verb=eachverbchild.nodeName
							if verb.find('#text')<0:
								#print verb
								verblst.append(verb)
			tempNP=father
		if len(verblstnormal(verblst))>0:
			nplist.append([NP,verblstnormal(verblst)])
			#print '\n'
...
if __name__=="__main__":
  ...
  #result=Map()
  #newparse_xml('/home/zyqu/newnewstanfordxml/air.A3DChartsMobile.apk/new56.xml')
  ...
  #print intersectpermissiondict
  #relatedictfile=open(proj_path+'/data/22458_100_0_67_relateddict.json','r')
  #replaceworddict = json.load(relatedictfile)
  #relatedictfile.close()
  xmlrootpath=proj_path+'/androidDS/allstdxml/'
  newparseNPverbpair(xmlrootpath)

...
def newgetverbNPpair(xmlrootpath,NPtocheck,permdict,replaceworddict,trainapplst,fold):
  ...
  for eachPerm in NPtocheck:
			...
			for Nroot, Ndirs, Nfiles in new_list_dirs:
				for Nfile in Nfiles:
					...
					for eachelem in npvblst:
						...
						verblst=eachelem[1]
						tempdict={}
						tempdict[app]=[npwithtyp]
						thisnp=[]
						if len(onlysinglenoun(tempdict).keys())>0:
							thisnp=onlysinglenoun(tempdict)[app]
						if len(twonouns(tempdict).keys())>0:
							thisnp.extend(twonouns(tempdict)[app])
						if len(adjnoun(tempdict).keys())>0:
							thisnp.extend(adjnoun(tempdict)[app])
						if len(thisnp)==0:
							continue
	...
	respath=proj_path+'/evaluation/newpotentVerbNP/'
...
def Batchparse_xml(xmlrootpath):
    ...
    file_op1=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/49183FullTypePatterns.txt','w')
    try:
...
def highlowfrq(freqdict,threshold):
	...
	print maxfreq
        ...
        file_op1=open('/home/zyqu/Research/Android_sec/parsexml/semant/grouping/dbscan/49183_wordsmore%s'%threshold+'.txt','w')
        try:
        ...
        file_op1=open('/home/zyqu/Research/Android_sec/parsexml/semant/grouping/dbscan/49183_wordsless%s'%threshold+'.txt','w')
        try:
...
def getwordfreq(keyworddict):
	...
	file_op1=open('/home/zyqu/Research/Android_sec/parsexml/semant/grouping/dbscan/49183_wordfreq.json','w')
	try:
...
if __name__=="__main__":
  ...
  topNPpath=proj_path+'/evaluation/topkNP/'
  ...
  for root, dirs, files in list_dirs:
        for d in dirs:
		    ...
		    NPfilepath=topNPpath+targetperm+'/selectedNPfold%s.json'%fold
		    NPfile=open(NPfilepath,'r')
		    NPtocheck=json.load(NPfile)
  		    NPfile.close()
		    newgetverbNPpair(xmlrootpath,NPtocheck,intersectpermissiondict,replaceworddict,trainapplst,fold)

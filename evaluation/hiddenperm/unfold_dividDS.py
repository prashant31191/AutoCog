...
if __name__=="__main__":
  ...
  filein=open('/home/zyqu/semant2/data/49183_apps_descdict.json','r')
  keyworddict2=json.load(filein)
  filein.close()
  print len(keyworddict2)

  filein=open('/home/zyqu/semant2/data/49183_apps_permdict.json','r')
  ...
  rootDir=validpath
  list_dirs=os.walk(rootDir)
  trainpath='/home/zyqu/semant2/evaluation/training/'

  for root, dirs, files in list_dirs:
        for d in dirs:
            validset=[]
            training=[]
            perm=d.replace('android.permission.','')
            validset=json.load(open(validpath+d+'/validset0.json','r'))

            print perm
            for apkName in validset:
                if apkName not in intersectpermissiondict:
                    print "Not found: "+ apkName

            for apkName in intersectpermissiondict:
                if apkName in validset:
                    continue
                training.append(apkName)
            print len(training)

            command='mkdir '+trainpath+d
            #print command
            #os.system(command)


            open(trainpath+d+'/trainset0.json','w').write(json.dumps(training))

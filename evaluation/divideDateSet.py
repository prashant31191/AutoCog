...
def newnewfscore(permdict,keyworddict,replaceworddict):
  TPdict={}
  FPdict={}
  FNdict={}
  Precisiondict={}
  ...
  numofapps=len(keyworddict)

  counter=0
  for permindex in range(24,len(permlist)):
  ...
  #frequency measurement for grouped words
	for eachgroupkey in replaceworddict:
		...
		if Recall/percentkwgroup <1.0 or Precision/percentperm<1.0:
			continue
		beta=0.2*(Recall/percentkwgroup)/(Precision/percentperm)
		#print (Recall/percentkwgroup)
		#print (Precision/percentperm)
		#print beta
		#fscore=2*Precision*Recall*1.0/(Precision+Recall)
		fscore=(1+beta*beta)*Precision*Recall/(beta*beta*Precision+Recall)
		tempdict[eachgroupkey]=fscore
  ...
  file_op=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/softappendres/beta_adj_75_25_Perm24to28fscgroupFeature.txt','w')
  try:
...
def getverbNPpair(xmlrootpath,NPtocheck,permdict,replaceworddict):

  loopcounter=0
  for eachPerm in NPtocheck:
	...
	for eachNP in NPtocheck[eachPerm]:
		relatedNPs=[]
		for eachrelatedNP in replaceworddict[eachNP]:
			if eachrelatedNP[0] not in relatedNPs:
				relatedNPs.append(eachrelatedNP[0])
		NPdict[eachNP]=relatedNPs
	...
	dircounter=0
    	for root, dirs, files in list_dirs:
		for d in dirs:  
			print "Finish processing %s"%dircounter+"in %s"%len(dirs)
			...
			if app not in permdict:
				continue

			hasperm= eachPerm in permdict[app]
			...
			for Nroot, Ndirs, Nfiles in new_list_dirs:
				for Nfile in Nfiles:
					...
					if len(parse_xml(filefullpath))==0:
						continue

					tempdict[app]=parse_xml(filefullpath)
					npinsent=[]

					#print tempdict[app]
					if len(onlysinglenoun(tempdict).keys())>0:
						npinsent=onlysinglenoun(tempdict)[app]

					if len(twonouns(tempdict).keys())>0:
						npinsent.extend(twonouns(tempdict)[app])

					if len(adjnoun(tempdict).keys())>0:
						npinsent.extend(adjnoun(tempdict)[app])
				
					if len(npinsent)==0:
						continue

					if len(verb_parse(filefullpath))==0:
						continue
					verbinsent=verb_parse(filefullpath)
					
					targetNP=[]
					for NPextracted in npinsent:
						for NPindict in NPdict:
							if (NPextracted==NPindict or NPextracted in NPdict[NPindict]) and NPindict not in targetNP:
								targetNP.append(NPindict)
					if len(targetNP)==0:
						continue
					...
					for eachelem in targetNP:
						if eachelem not in Resdict:
							Resdict[eachelem]=[]

						for eachtargetverb in verbinsent:
							if len(Resdict[eachelem])==0:
								if hasperm==True:
									Resdict[eachelem].append([eachtargetverb,1,0])
								elif hasperm==False:
									Resdict[eachelem].append([eachtargetverb,0,1])
								continue
							hasverb=False 
							for eachverbDS in Resdict[eachelem]:
								if eachtargetverb==eachverbDS[0]:
									hasverb=True
									if hasperm==True:
										eachverbDS[1]=eachverbDS[1]+1
									elif hasperm==False:
										eachverbDS[2]=eachverbDS[2]+1
									break
							if hasverb==False:
								if hasperm==True:
									Resdict[eachelem].append([eachtargetverb,1,0])	
    	...
    	for collectedNP in Resdict:
		tempdict={}
		for collectedverb in Resdict[collectedNP]:
			if collectedverb[0] not in tempdict:
				tempdict[collectedverb[0]]=collectedverb[1]*1.0/(collectedverb[1]+collectedverb[2])
		finalsorteddict[collectedNP]=sorted(tempdict.iteritems(), key=lambda d:d[1], reverse=True)

	f=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/verbNP/200_'+eachPerm+'.txt','w')
	try:
		for eachelemNP in finalsorteddict:
			f.write(eachelemNP+'\n')
			for eachelemvb in finalsorteddict[eachelemNP]:

				total=0
				for targetvb in Resdict[eachelemNP]:
					if eachelemvb[0]==targetvb[0]:
						total=targetvb[1]+targetvb[2]

				f.write(eachelemvb[0]+'\t%s'%eachelemvb[1]+'\t%s\n'%total)
...
def Batchparse_xml(xmlrootpath):
    ...
    for root, dirs, files in list_dirs:
	for d in dirs:       
		#print "Finished %s apps in "%counter+"%s"%len(dirs)
		counter=counter+1
		level2rootDir=os.path.join(root, d)
		app=os.path.basename(level2rootDir)
		newroot=rootDir+app+'/'
    ...
    file_op1=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/22458FullTypePatterns.txt','w')
    try:
			...
			if wordtype1=='J' and wordtype2=='N':
				if word1.find('/')<0 and word2.find('/')<0:
					...
					for tempword1 in word1list:
						tempword1=tempword1.strip()
						if (tempword1 not in filteredlist) and (tempword1 not in stopwords):
							tempword1=lemmatizer.lemmatize(tempword1,'a')
							for originalword in replacedict:
								if tempword1 in replacedict[originalword]:
									tempword1=originalword
						elif (tempword1 in filteredlist) or (tempword1 in stopwords):
...
def BatchpreprocessxmlT(xmlrootpath):
	...
	for d in dirs:       
		level2rootDir=os.path.join(root, d)
		app=os.path.basename(level2rootDir)
		print "Apps %s in "%counter+"%s"%len(dirs)
		counter=counter+1
		newroot=rootDir+app+'/'
		new_list_dirs=os.walk(newroot)
		for Nroot, Ndirs, Nfiles in new_list_dirs:
...
def traves(node,wordlst):
    from nltk.corpus import stopwords
    stopwords = stopwords.words('english')
    lemmatizer = nltk.WordNetLemmatizer()

    if len(node.childNodes)==0:
	typ=node.parentNode.nodeName
        word=node.nodeName
	word=word.replace('#text','')
	word=word.strip()
	if typ.startswith('V') and len(word)>0 and word.lower() not in stopwords and word.lower() not in filteredlist:
		if word.find('XXXXX')>0:
			tempwordlst=word.split('XXXXX')
			for eachword in tempwordlst:
				if len(eachword)>0 and eachword.lower() not in stopwords and eachword.lower() not in filteredlist:
					wordtoappend=eachword.lower()
					wordtoappend=lemmatizer.lemmatize(wordtoappend,'v')
					wordlst.append(wordtoappend)

		elif word.find('XXXXX')<0:
			word=word.lower()
...
def removeredunt(lst):
	mainlst=[]
	for elem in lst:
		if elem not in mainlst and len(elem)>0:
			mainlst.append(elem)
	return mainlst
...
def highlowfrq(freqdict,threshold):

	morelst=[]
        lesslst=[]
	maxfreq=0
	counter=1
        for eachwd in freqdict:
		print "%s in "%counter+"%s"%len(freqdict)
		counter=counter+1
		if freqdict[eachwd]>maxfreq:
			maxfreq=freqdict[eachwd]
                if eachwd not in morelst and freqdict[eachwd]>=threshold:
                        morelst.append(eachwd)
                if eachwd not in lesslst and freqdict[eachwd]<threshold:
                        lesslst.append(eachwd)
        print len(morelst)
        print len(lesslst)
	print maxfreq


        file_op1=open('/home/zyqu/Research/Android_sec/parsexml/semant/grouping/dbscan/22458_wordsmore%s'%threshold+'.txt','w')
        try:
                for wdindex in range(len(morelst)):
                        file_op1.write(morelst[wdindex]+'\t%s\n'%wdindex)
        finally:
                file_op1.close()

        file_op1=open('/home/zyqu/Research/Android_sec/parsexml/semant/grouping/dbscan/22458_wordsless%s'%threshold+'.txt','w')
        try:
                for wdindex in range(len(lesslst)):
                        file_op1.write(lesslst[wdindex]+'\t%s\n'%wdindex)
        finally:
                file_op1.close()
...
def getwordfreq(keyworddict):
	wordlst=[]
	freqdict={}
	counter=1
	for eachApk in keyworddict:
		print "the %s app in "%counter+"%s"%len(keyworddict)
		counter=counter+1
		for eachword in keyworddict[eachApk]:
			if eachword not in wordlst:
				wordlst.append(eachword)

			if eachword not in freqdict:
				freqdict[eachword]=1
				continue

			if eachword in freqdict:
				freqdict[eachword]=freqdict[eachword]+1


	file_op1=open('/home/zyqu/Research/Android_sec/parsexml/semant/grouping/dbscan/22458_wordfreq.json','w')
	try:
		file_op1.write(json.dumps(freqdict))
	finally:
		file_op1.close()
	#file_op2=open('tempfreq.txt','w')
	#try:
		#for eachwd in freqdict:
			#file_op2.write('%s\n'%freqdict[eachwd])
			#if freqdict[eachwd]==1:
				#print eachwd
	#finally:
		#file_op2.close()
...
if __name__=="__main__":
  ...
  filein=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/22458_apps_descdict.json','r')
  keyworddict2=json.load(filein)
  filein.close()
  print len(keyworddict2)

  filein=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/22458_apps_permdict.json','r')
  ...
  permlst=[]
  for apkName in intersectpermissiondict:
 	for eachPerm in intersectpermissiondict[apkName]:
    		if eachPerm not in permlst:
      			permlst.append(eachPerm)
  ...
  for eachPerm in permlst:
	print eachPerm
  	...
  	appswithoutperm=[]
  	for apkName in intersectpermissiondict:
    		if eachPerm in intersectpermissiondict[apkName]:
			if apkName not in appswithperm:
      				appswithperm.append(apkName)
    		elif eachPerm not in intersectpermissiondict[apkName]:
			if apkName not in appswithoutperm:
      				appswithoutperm.append(apkName)
	trainpath='/home/zyqu/Research/Android_sec/parsexml/semant/evaluation/training/'+eachPerm
  	command='mkdir '+trainpath
	os.system(command)
	validpath='/home/zyqu/Research/Android_sec/parsexml/semant/evaluation/valiation/'+eachPerm
  	command='mkdir '+validpath
	os.system(command)
	foldcounter=0
	seg1=len(appswithperm)/folder
	seg2=len(appswithoutperm)/folder
	while foldcounter<folder:
		validapps=[]
		trainapps=[]

		if foldcounter<folder-1:
			validapps=appswithperm[foldcounter*seg1:(foldcounter+1)*seg1]
			validapps.extend(appswithoutperm[foldcounter*seg2:(foldcounter+1)*seg2])
			


		elif foldcounter==folder-1:
			validapps=appswithperm[foldcounter*seg1:len(appswithperm)]
			validapps.extend(appswithoutperm[foldcounter*seg2:len(appswithoutperm)])

		for eachApk in intersectpermissiondict:
			if eachApk in validapps:
				continue
			if eachApk not in trainapps:
				trainapps.append(eachApk)
		...
		trainfile=trainpath+'/trainset%s.json'%foldcounter
		validfile=validpath+'/validset%s.json'%foldcounter

		f=open(trainfile,'w')
		f.write(json.dumps(trainapps))
		f.close()
		
		f=open(validfile,'w')
		f.write(json.dumps(validapps))
		f.close()
  ...
  #verb_parse('/home/zyqu/newnewstanfordxml/zjlwp.football.star.live.wallpaper.apk/new12.xml')
  #result=Map()
  #BatchpreprocessxmlT('/home/zyqu/newnewstanfordxml/')
  #phrasedict=Batchparse_xml('/home/zyqu/newnewstanfordxml/')

  #AllN=onlysinglenoun(phrasedict)
  #fileout=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/22458FullN.json','w')
  #fileout.write(json.dumps(AllN))
  #fileout.close()


  #AllNN=twonouns(phrasedict)
  #fileout=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/22458FullN+N.json','w')
  #fileout.write(json.dumps(AllNN))
  #fileout.close()

  #AllJN=adjnoun(phrasedict)
  #fileout=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/22458FullJN.json','w')
  #fileout.write(json.dumps(AllJN))
  #fileout.close()


  #filein=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/22458FullJN.json','r')
  #keyworddict21=json.load(filein)
  #filein.close()

  #filein=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/22458FullN+N.json','r')
  #keyworddict22=json.load(filein)
  #filein.close()

  #filein=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/22458FullN.json','r')
  #keyworddict23=json.load(filein)
  #filein.close()

  #keyworddict3=dictmerge(keyworddict21,keyworddict22)
  #keyworddict3=dictmerge(keyworddict3,keyworddict23):
  #keyworddict2={}
  #for eachApk in keyworddict3:
	#if len(keyworddict3[eachApk])>0:
		#keyworddict2[eachApk]=keyworddict3[eachApk]

  #fileo=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/22458_apps_descdict.json','w')
  #fileo.write(json.dumps(keyworddict2))
  #fileo.close()
  #print len(keyworddict2)
  #f=open('/home/zyqu/Research/Android_sec/parsexml/semant/data/22458_apps_descdict.json','r')
  #keyworddict2=json.load(f)
  #print len(keyworddict2)
  #getwordfreq(keyworddict2)
		
  #filein=open('/home/zyqu/Research/Android_sec/parsexml/semant/grouping/dbscan/22458_wordfreq.json','r')
  #freqdict=json.load(filein)
  #filein.close()
  #threshold=25
  #highlowfrq(freqdict,threshold)
